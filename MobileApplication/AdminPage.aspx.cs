    ...
    public partial class AdminPage : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                foreach (DataRow row in mobileTable.Rows)
                {
                    ...
                    button.CssClass = "w-full mt-2 px-4 py-2 bg-yellow-500 text-white shadow-md border rounded-md cursor-pointer";

                    button.Click += (s, args) =>
                    {
                        Response.Redirect("MobileDetails.aspx?mobileId=" + row["MobileId"].ToString());
                    };
        ...
        protected DataTable FetchMobileDataFromDatabase()
        {
            ...
            using (SqlConnection connection = new SqlConnection(conn))
            {
                ...
                string query = "SELECT Brand, ModelName, Price, ImageUrl, RAM, Storage FROM Mobile";
